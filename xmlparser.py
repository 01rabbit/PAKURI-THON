    ...
    def parse_nmap_xml(self,root):
        ...
        for host in hosts:
            ...
            if host_id is not None:
                ...
                host_id = self.db.insert_db(sql, arg)
                if host_id != "None":
                    sql = """UPDATE t_host_list SET state = %s, timestamp = %s WHERE id = %s;"""
                    arg = (state, time, host_id)
                    self.db.update_db(sql, arg)
            ...
            try:
                ...
                for port in ports:
                    ...
                    port_id=self.db.get_SingleValue(sql,portid)
                    if port_id is not None:
                        ...
                        port_id = self.db.insert_db(sql, arg)
                        if port_id != "None":
                    ...
                    for script in script_element:
                        ...
                        vuln_id = self.db.get_SingleValue(sql, script_id)
                        if vuln_id is None:
                            ...
                            vuln_id = self.db.insert_db(sql, arg)
            except IndexError:
                continue

    def parse_nikto_xml(self,root):
        ...
        time = self.get_timestamp()
        sql = """SELECT id FROM t_host_list WHERE ip_address = %s;"""
        host_id = self.db.get_SingleValue(sql, targetip)
        sql = """SELECT id FROM t_port_list WHERE host_id = """ + str(host_id)+""" AND port_num = %s;"""
        port_id = self.db.get_SingleValue(sql, targetport)
        ...
        for item in items:
            ...
            sql = """SELECT id FROM t_vuln_list WHERE port_id = """+str(port_id)+""" AND script_id = %s;"""
            vuln_id = self.db.get_SingleValue(sql, item_id)
            if vuln_id is None:
                sql = """INSERT INTO t_vuln_list(port_id,script_id,output,timestamp) VALUES(%s,%s,%s,%s) RETURNING id;"""
...
if __name__ == "__main__":
    try:
        ...
        if os.path.isfile(scanfile):
            tree = parser.createTree(scanfile)
            if tree.tag == "nmaprun":
                parser.parse_nmap_xml(tree)
            elif tree.tag == "niktoscan":
                parser.parse_nikto_xml(tree)
            else:
    ...
    parser.mc.botbot_information(attachment)
