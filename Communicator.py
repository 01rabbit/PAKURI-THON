...
def check_ipaddress(ip):
    ...
    Returns:
        bool : IPアドレスの形式になっていればtrue、そうでなければfalse
    """
    try:
...
def ChatCommunication():
        ...
        if len (strArry) == 3:
            ...
            if args == "help":
                if command == "nmap" or command == "nikto":
                    attachment = cmd_help(posted_user,command)
                elif command == "tools":
                    attachment = tools_help(posted_user)
                else:
            ...
            for selectCmd in cmdList:
                ...
                if cmdName == cmdType:
                    filename = f"{cmdName}_{target}_" + get_timestamp()
                    if command == "nmap" or command == "tools":
                        if check_ipaddress(target):
                            # Use Nmap
                            if command == "nmap":
                            ...
                            attachment = ipaddress_error(posted_user,target)
                    elif command == "nikto":
    ...
    # Show host information
        ...
        if len(strArry) == 3:
            target = strArry[2]
            if target == "help":
                attachment = info_help(posted_user,"help")
            else:
                if check_ipaddress(target):
                    sql = """SELECT id FROM t_host_list WHERE ip_address = %s;"""
                    host_id = db.get_SingleValue(sql, target)
                    if host_id is None:
                        attachment = info_help(posted_user,"norecord")
                    else:
                        attachment = hostinfo(host_id)
                else:
                    attachment = ipaddress_error(posted_user,target)
        else:
            attachment = info_help(posted_user,"error")
